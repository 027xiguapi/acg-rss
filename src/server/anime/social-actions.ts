...
interface ReactConfig {
  ...
  /** Insert row keyed by the target column (animeId vs episodeId) */
...
const FAVORITES: Record<SocialKind, ReactConfig> = {
  anime: {
    ...
    makeRow: (targetId, userId) => ({ animeId: targetId, userId }),
  },
  episode: {
...
const COMMENTS: Record<SocialKind, CommentConfig> = {
  anime: {
    table: animeComments,
    targetCol: animeComments.animeId,
    makeRow: (targetId, userId, content) => ({ animeId: targetId, userId, content }),
  },
  episode: {
...
/** Add or remove the current user's favorite on one anime or episode. */
...
/** Add or remove the current user's like on one anime or episode. */
