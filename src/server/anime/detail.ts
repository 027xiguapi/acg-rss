...
import { db } from "@/db";
import { anime, animeEpisodes, animeInfos, torrentItems } from "@/db/schema";
import type { AnimeWithTitle, TorrentItem } from "@/db/schema";
import { withTitles } from "@/server/anime/queries";

/** One anime episode with every release variant collected for it. */
...
/** Everything the anime detail page renders, derived in one place. */
export interface AnimeDetail {
  ...
  /** Torrents linked to this anime but without a parsed episode number. */
...
/** An anime row with its primary display name resolved from anime_infos. */
export async function loadAnime(id: number): Promise<AnimeWithTitle | null> {
  const rows = await db.select().from(anime).where(eq(anime.id, id)).limit(1);
  const row = rows[0];
  if (!row) return null;
  const primary = await db
    .select({ title: animeInfos.title })
    .from(animeInfos)
    .where(and(eq(animeInfos.animeId, id), eq(animeInfos.kind, "primary")))
    .limit(1);
  return { ...row, title: primary[0]?.title ?? "" };
}

export async function loadAnimeDetail(animeId: number): Promise<AnimeDetail | null> {
  const item = await loadAnime(animeId);
  if (!item) return null;

  // Episodes with their torrents (newest first inside each episode)
  const episodeRows = await db
    .select({
      episodeId: animeEpisodes.id,
      number: animeEpisodes.number,
      torrent: torrentItems,
    })
    .from(animeEpisodes)
    .leftJoin(torrentItems, eq(torrentItems.episodeId, animeEpisodes.id))
    .where(eq(animeEpisodes.animeId, animeId))
    .orderBy(
      desc(animeEpisodes.number),
      desc(torrentItems.publishTime),
      desc(torrentItems.createdAt)
    );

  // Linked to this anime but no parsed episode number
  const unparsed = await db
    .select()
    .from(torrentItems)
    .where(and(eq(torrentItems.animeId, animeId), isNull(torrentItems.episodeId)))
  ...
  const [totals, titleRows, otherRows, latestRows] = await Promise.all([
    db
      ...
      .from(torrentItems)
      .where(eq(torrentItems.animeId, animeId)),
    // Structured names for the basic info card
    db.select().from(animeInfos).where(eq(animeInfos.animeId, animeId)),
    // Other tracked series feed the related-recommendations grid
    db.select().from(anime).where(ne(anime.id, animeId)),
    db
      .select({ animeId: animeEpisodes.animeId, latest: max(animeEpisodes.number) })
      .from(animeEpisodes)
  ...
  const latestMap = new Map(latestRows.map((row) => [row.animeId, row.latest]));
