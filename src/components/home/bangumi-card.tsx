...
import { MessageSquare, RefreshCw, Tv } from "lucide-react";
...
import { BangumiSubscribeButton } from "@/components/home/bangumi-subscribe-button";
...
export async function BangumiCard({ entry }: { entry: BangumiCardData }) {
  const { item, latest, coverName, commentCount, torrentCount } = entry;
        ...
        <div className="flex flex-col gap-1 px-0.5">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              ...
              {tHome("commentCount", { count: commentCount })}
            </span>
            <span className="inline-flex items-center gap-1">
              <RefreshCw className="size-3.5" />
              {tHome("updateCount", { count: torrentCount })}
            </span>
          </div>
        </div>
      </Link>
      <BangumiSubscribeButton
        bangumiId={item.id}
        title={item.title}
